...
<div class="space-y-6 max-w-4xl">
    ...
    <div class="flex items-center justify-between">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white font-sans">Write New Blog Article</h2>
    ...
    <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 sm:p-8 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="e.g. 5 Ways QR Codes Can Grow Your Business" class="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <!-- Category -->
            <div>
                ...
                <div class="grid grid-cols-2 gap-2 text-xs">
                    @foreach($tags as $tag)
                        <div class="flex items-center">
                            <input type="checkbox" name="tags[]" value="{{ $tag->id }}" id="tag-{{ $tag->id }}" {{ is_array(old('tags')) && in_array($tag->id, old('tags')) ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-slate-300 rounded" />
        ...
        <div x-data="{ seoOpen: false }" class="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
            ...
            <div x-show="seoOpen" class="p-5 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 space-y-4" x-transition>
                ...
                <div>
                    ...
                    <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords') }}" placeholder="e.g. qr codes, brand styling, guide" class="w-full px-3 py-2 text-xs rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-slate-100 dark:border-slate-700/50 pt-6 flex items-center justify-between">
            <div class="flex items-center">
                <input type="checkbox" name="is_published" value="1" id="is_published" {{ old('is_published') ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-slate-300 rounded" />
                <label for="is_published" class="ml-2 block text-xs font-bold text-slate-900 dark:text-white select-none">Publish Immediately</label>
            </div>
